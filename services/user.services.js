...
exports.updatePasswordService = async (id, reqData) => {
  const user = await User.findById(id);
  if (reqData) {
    user.password = reqData;
...
exports.forgetPasswordService = async (email) => {
  ...
  if (!user) {
    ...
    const resetURL = `Hello, please follow this link to reset your password. This link is valid till 10 minutes form now. <a href="http:localhost:4000/api/v1/user/reset-password/${token}">Reset Password</a>`;
    const data = {
      to: email,
                                    ...
                                    <tr>
                                      <td class="v-container-padding-padding"
                                        ...
                                        <div class="v-text-align" align="left">
                                          ...
                                          <a href="http:localhost:4000/api/v1/user/reset-password/${token}" target="_blank" class="v-button"
...
exports.resetPasswordService = async (token, password) => {
  const hashedToken = crypto
    // .createHash(process.env.CRYPTO_KEY)
    .createHash("sha256")
    .update(token)
    .digest("hex");
